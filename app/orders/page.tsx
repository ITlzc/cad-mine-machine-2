...
export default function OrdersPage() {
  ...
  const getStatusTag = (status: number) => {
    switch (status) {
      ...
      case 7:
        return <span className="text-blue-600 bg-blue-50 px-2 py-1 rounded">机器已发货</span>
      case 8:
        return <span className="text-blue-600 bg-blue-50 px-2 py-1 rounded">机器已签收</span>
      case 9:
        return <span className="text-blue-600 bg-blue-50 px-2 py-1 rounded">机器待托管</span>
      case 10:
        return <span className="text-blue-600 bg-blue-50 px-2 py-1 rounded">机器已托管</span>
  ...
  const handleTransfer = async (toastId: string, paymentAddress: string, amount: string, orderId: string) => {
    try {
      ...
      // 使用ethers.utils.parseUnits将金额转换为wei（18位小数）
      const amountInWei = BigInt(ethers.utils.parseUnits(amount, 18).toString())
    ...
    <>
      <div className="min-h-screen bg-gray-50">
        <div className="mx-auto p-4">
          {!showOrderDetail && <div className="flex justify-between items-center mb-6">
          ...
          {loading && !showOrderDetail ? <Loading /> : (
            <>
              {showOrderDetail ? (
                <OrderDetail
                  order={selectedOrder}
                  ...
                  userInfo={userInfo}
                  upList={()=>fetchOrders(pagination.current, searchTerm)}
                />
              ) : (
                            ...
                            <tr key={order.id} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-4">
                                <div className="flex flex-wrap gap-2">
                                  ...
                                  <button
                                    className="px-3 w-[90px] py-1.5 border border-gray-300 text-gray-700 text-sm rounded hover:bg-gray-50 transition-colors"
